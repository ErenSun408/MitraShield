...
- **握手双向确认**：B 在公钥行尾附 ` p2` 标记 → 发一行 `AES-GCM(K, "MIDUN-CONNECTOR-OK")` 证明自己算出了同一把密钥（即确实持有二维码里的 A 公钥）→ A 验过后回一行 `AES-GCM(K, "MIDUN-LISTENER-OK")`。**B 收到这一行才算连上**，它证明的是「A 真的 accept 并读到了我」，而不只是「TCP 通了」。
- **A 侧常驻监听**：accept 循环搬进单例 scope（`listenerJob`），会话断了继续 accept，对端拿原邀请码重连即接上；新连入顶掉旧会话。**顶替以握手确认为前提**——连到 8888 的人不必知道 A 的公钥也能让 A 的 ECDH 走完，光「握手成功」不构成身份证明；老版本对端（无确认）只在本机空闲时接纳，避免任意连入就能踢掉会话。
- **文件收妥回执**：新增 `FILE_ACK` 帧（`{msgId, ok}`，会话密钥加密），收端校验完 sha256 后发出，发端等到它才标「已送达」；等不到（30s）即标失败并弃用当前通道。

**向后兼容是完整的**：老 B 扫新 A 的码——老 B 不看 `ver`、只发纯公钥，新 A 由「公钥行无 ` p2` 后缀」判定为老版本，两步确认与回执等待一律跳过，线上字节与 v1 逐字节相同；新 B 扫老 A 的码——`ver`=1，新 B 同样退回 v1 路径。`FILE_ACK` 对老对端是未知帧类型，收到即忽略。
