...
- **遗留 / M11 收口**
  ...
  - **HKDF salt 取全零**：每会话临时密钥新生（`generateConnectionInfo` 每次新对）已保证会话密钥不复用，故未把 `sessionId` 编入 salt；如需更强会话绑定可后续把 sid 作 salt。**（2026-08-14 更新：该路线作废，`sid` 已随邀请码载荷混淆一并删除，理由见文末「邀请码载荷混淆」一节。）**
...
## 波波换皮（客户改造，v4 外，`bobo` 分支，2026-07-09）

甲方要求把「密盾」安全工具外观整体换成「波波」社交式换皮（品牌/伪装需求，功能内核不动）。改造清单源自客户 `修改.xlsx`（两 sheet + 标注截图 + 两张 AI 设计稿）；两 sheet 命名有冲突，以**标注截图（客户在真实首页截图上手写红字）= Sheet1 A9 方案**为准，Sheet2 为早期草稿作废。全部在 `bobo` 分支、逐子项一 commit。

- **品牌文案 `[rebrand]` `44dd94a`**：`app_name` 密盾→**波波**（桌面名 + 应用内全部「密盾」字样：登录页/启动页/关于页/设置/扫码错误提示/加密导出文案）；启动页标语「您的安全由我们守护」→「愿我们在平淡里听见彼此的声音」。内部注释/`Theme.MiDun` 样式 ID 不改。
- **菜单/标题改名 + 去副标题 `[rebrand]` `b6ef1b8`**：USB安全卡→**守波秘钥**、即时通信→**相逢叙话**、隐私文件夹→**私藏清隅**、扫码建链→**同波相契**、一键清理→**涤净闲存**；底部导航 文件夹→**私藏**、通信→**波一下**。去掉设备卡「文件数量」统计、快捷卡副标题「0个文件夹/安全连接/安全销毁」（`QuickActionCard.subtitle` 改可空、非空才渲染）。范围=首页快捷卡/设备卡 + 底部导航 + 各屏顶栏标题 + 设置菜单项 + 操作日志 `CONNECT` 标签。
- **散文术语自然改写 `[rebrand]` `27732b2`**（用户 2026-07-09 定「自然改写」）：句子正文里的旧功能词改成通顺中性词，既不露旧词、读着也自然——`USB安全卡/安全卡`→**设备**、`隐私文件夹`→**文件夹**、`隐私文件`→**文件**，覆盖启动/登录/初始化/文件/设置/聊天/拔卡遮罩 41 处显示字符串。诗意名只作菜单标签（塞进「请重新插入守波秘钥」类句子会生硬），故句中不用诗意名。脚本按「非注释行」替换（中文词在非注释行只出现于字符串字面量 → 安全），注释/KDoc（含里程碑引用）不动。
- **未插卡首页背景 `[rebrand]` `9705512`**：原纯白等待屏（`status==DISCONNECTED && !isTestMode`）铺客户蓝色水墨图（`drawable-nodpi/nocard_bg.jpg`，底部「豆包AI生成」水印已裁掉、缩到 1080 宽）。仅无卡等待态显示，有卡各屏保留自身背景。
- **App 图标 `[rebrand]` `ded591c`**：换成客户「波波」双气泡图标。原图是白底画布上的圆角方图标 → 裁到图标框、把**连通的外白/圆角洪水填充设为透明**（气泡内部白色高光不与外白连通、不受影响）作 adaptive 前景（`drawable-nodpi/bobo_icon_fg.png`）；背景改纯深 navy（`#021C43`）兜底 → 任意蒙版形状无缝。删旧「Security 盾牌」矢量前景 + monochrome 层（全彩图不适合主题图标染色）。仍 adaptive-only（minSdk24 上 7.x 显通用图标，用户既定不管 7.x）。
- **阅后即焚时间 + 去墓碑 `[chat]` `e1d6783`**：焚毁时长选项 5/30/60/300 秒 → **10/15/30/60 秒**（`formatTtl` 通用 `<60→N秒/否则 N分钟`，无需改）。**去掉焚毁墓碑**「🔥 阅后即焚消息已焚毁」（用户 Q2 只去这一条，撤回墓碑等保留）：`displayMessages` 过滤 `burned` 消息 → 焚毁后整条消失不留痕（比空占位更干净、无孤立时间分隔行）。
- **二维码分享白底 `[qr]` `a68e59d`**：`QRCode.render()` 产透明底 PNG，分享到部分机型被填黑 → 生成 bitmap 时 `Canvas` 压到不透明白底（`drawColor(WHITE)` + `drawBitmap`）→ 屏显与分享 PNG 均白底。
...
## 安全卡「公开区」（明文分区）调研结论（无改码，2026-07-15）

老板提问「卡里除了加密分区还有个未加密的正常分区，我们对它有什么操作」引出的调研。**结论：我们对公开区零操作，且这不是疏漏、也基本无法改变。** 本节只记录事实与边界，不涉及代码改动。

### 分区结构（厂商《技术白皮书》§1.1 / 《产品介绍》§3.1）

U 盘物理上分三个功能区，出厂三选一种分区模式：

| 分区 | 类型 | 可访问性 |
|---|---|---|
| 光盘区 | CDROM，可烧 ISO | 系统标准指令识别，只读 |
| 公开区 | 普通可移动磁盘 | **无密码、无加密，任何人插上即可读写** |
| 隐藏区 | 自研 SZFS | 仅 FSShell API + 密码认证 |

分区模式：①光盘区+公开区+隐藏区 ②光盘区+隐藏区 ③公开区+隐藏区。容量划分由厂商烧录工具（`SDK/Tools/*/光盘更新工具` 等）在出厂时定，**App 无法更改**。隐藏区之所以「隐藏」，是它走**非标准 SCSI 指令集**，操作系统不认；光盘区/公开区走标准指令，故系统自动挂载。

（`FSShell JAVA SDK 开发文档` 与 Linux 指引里零星出现过「安全区」一词，但正式的分区结构/模式列表中没有，存疑，必要时问厂商。）

### 本仓库对公开区的操作：**无**

全仓库卡内路径均为 `0:/` 前缀，而 `0:/` 在 SDK 语义中是**隐藏区的 0 号分区**，与公开区无关：

- `RealFileSystem.kt` `ROOT = "0:/"`；元数据/keystore 均在 `0:/.midun_*`
- `RealUsbManager.kt` 容量读 `SFGetCapacity("0:/")` → **仅隐藏区容量**（另有 `SFDiskGetCapacity` 是「含光盘区/公开区/隐藏区」的整盘容量，本仓库**未用**，故首页/设置显示的容量不含公开区，符合预期）
- `RealUsbManager.kt` 恢复出厂 `SFFormat(ROOT)` → **仅格式化隐藏区**

FSShell API 本身也**不提供任何公开区接口**——它只管隐藏区。读写公开区必须走系统普通存储框架（mass storage / SAF），与本 SDK 无关。

### `USBStorageHelper.Open()` 会卸载公开区（SDK 通用行为）

Android SDK 文档 `Open` 备注原文：「如果 USB 设备已经挂载，则在打开成功后会被自动卸载」「调用 open 方法成功后……无法在文件浏览器中查看优盘公开区的文件」；`Close` 备注：「关闭成功后设备可能会被系统自动挂载」。

机制（`seczure.fsudisk.fscommon.jar` 中 `USBStorageHelper.class` 常量池佐证）：调用链为 `UsbManager.openDevice()` → `claimInterface(UsbInterface, boolean)`（含 force 双参版本，另有 `releaseInterface` / `bClaimIntf` / `getFileDescriptor`），fd 交 libusb；文档亦称「SDK 底层使用 libusb 对 usbfs 驱动进行操作」。

**为何无法规避**：本卡光盘区/公开区/隐藏区**共用同一个 USB Mass Storage 接口**、靠 LUN 区分（白皮书原理图「返回 LUN 数量，类型信息等」），而内核 `usb-storage` 驱动**绑定接口、不绑定 LUN**。USB 接口占用天生排他 → 二选一：内核占着（公开区可挂载、隐藏区指令发不出去）或我们 `claimInterface` 抢占（隐藏区可用、公开区被卸载）。没有「内核与用户态各管一个 LUN」的玩法。

理论出路只有两条，均不采用：①用完即 `Close()` 释放——但 `SFOpenDiskEx` = 开盘 + 密码认证是同一调用，释放后重占需**重新要密码**，UX 不可接受；②厂商定制固件拆成两个 USB 接口（接口 0 给光盘+公开区、接口 1 给隐藏区）则可共存（《产品介绍》称「支持定制 PCBA 模块」），但属硬件改造，超出本项目范围。

⚠️ **与既有记录的冲突，待真机验证**：上述是厂商**通用产品**的 SDK 文档描述。但本文档「移除『从普通U盘导入』入口（2026-07-01）」条已记「本安全卡的明文公开区是 PC 侧（USB-A）功能、手机端（Type-C 加密侧）本就看不到」，`requirements-traceability.md` 亦记「Type-C 加密U盘对 PC 不可见｜硬件特性」。**若本卡确为双接口（USB-A 明文侧 / Type-C 加密侧）分工，则手机端根本不存在可见的公开区，「Open 卸载公开区」在 Android 侧是空转、无实际影响。** 两说未经真机对照验证——验证方法：真卡插手机（Type-C），在 App 连卡**之前**用系统文件管理器看是否存在可浏览的 U盘 卷，连卡后再看是否消失。

### 真实影响：公开区不受 App 任何清理覆盖

**无论上述冲突结论如何，这一条都成立**：`SFFormat("0:/")` 只擦隐藏区，故「恢复出厂」「一键清理（涤净闲存）」「退出自动清理」**全部清不到公开区**。若用户（在 PC 侧）往公开区放过文件，点「恢复出厂」后那些文件原封不动还在——与当前的安全叙事存在落差。

App 内补做「清理公开区」**不现实**：须先完全释放 USB 句柄（≈退出登录）、等系统重新挂载（异步、秒级、时序不可控）、再走 SAF 让用户手动选，交互极差。

### 建议（**待老板拍板，尚未执行**）

**出厂改用分区模式 ②（光盘区+隐藏区），砍掉公开区。** 既消除明文泄密面，也消除「清理覆盖不到」的空白，还省掉向用户解释的成本。需与厂商确认烧录配置。若必须保留公开区，则应明确其用途（如仅放说明书/装机包）并在文案上与「安全存储」切割。

<!-- 后续里程碑的偏离继续在下面追加 -->

## 阅后即焚：发送方本端自焚（绝对 TTL）+ 焚毁死线持久化（v4 外增量，2026-07-23）

M10.8 定的焚毁模型是**读触发、双端同焚**：只有接收方点开→倒计时→到点发 `BURN` 帧，发送方那份**被动等这帧**才焚。客户实测暴露两个洞（bobo 真卡版）：

1. **发送方永久残留**：`BURN` 帧只在活动会话上写（`burnMessage` 的 `session != null && contactId 匹配` 分支），无待发队列、无重连补发。任一方下线 → 帧永远到不了 → 接收方正常焚、发送方那条永远停在「阅后即焚 · 对方读后焚毁」。M10.8 记的「离线退化：本端照常焚毁、对端收不到 BURN 帧」只覆盖了**读方**离线，没覆盖**发方**收不到。
2. **接收方重启复看**：`burnTimers` 只在内存，消息也无「已揭示」持久标记。倒计时途中进程被杀（划任务卡 / 息屏退出 `exitProcess` / 系统回收）→ 计时丢失且遮罩复原，**同一条明文可再看一次**、且永不焚。比 1 更严重（那是明文实体，`.recv_<id>` 还在卡上）。

**设计定案由用户拍板（2026-07-23）**：不做「BURN 待发队列 + 重连补发」（治不了对端永不再上线），改为**发送方自己持有死线**——认知前提是「删自己那份从不需要对端配合，`BURN` 帧只是『你可以删了』的通知，不是删除权限」。

- **`ChatMessage.burnDeadline`（epoch ms，可空）落卡侧车**：接收方点开时写 `now + burnTtl`，发送方发出时写 `now + 本端自焚时长`。
- **登录补焚**：`ChatRepository` 加载完卡内快照后发 `snapshotLoaded` 信号 → `P2PSessionManager.restoreBurnDeadlines()` 扫 `burnPending()`：已过期当场焚、未过期续挂计时。跑在快照就位之后、会话页渲染之前，故**不存在「能读到但还没焚」的窗口**。
- **时钟回拨保守判定**：`now < 消息 timestamp` 直接判过期焚掉（宁可早焚不可晚焚）。
- **`burnMessage(notifyPeer)`**：本端自焚传 `false`——本端到期**绝不能捎带删对端那份**，对方可能还没读，其阅读机会由接收端自己的遮罩 + 读后倒计时决定，其副本由对端自己的自焚死线管。
- **两段时长弹框**：读后倒计时（10/15/30秒/1分钟，默认 30 秒）+ 本端自焚（30秒/1分钟/10分钟，默认 1 分钟），加「开启」确认按钮。**本端自焚是纯本地设置、不入 `BURN_MODE` 帧**（payload 仍 `on:<读后倒计时>` / `off`），协议不变、与旧端兼容。
- **覆盖路径**：在线文字、文件/语音、离线文件发送。发送方气泡改显实时倒计时（超 60 秒按分钟排版）。

**认下的限制**：做不到「进程被杀后仍准点焚」——Android 上只能靠 AlarmManager/WorkManager 拉起，而那时卡多半没插、隐藏区也没开盘，照样删不了。故实际语义是**「到点即焚；若那时进程不在，则下次登录补焚」**。欠焚窗口内数据只存在卡的隐藏区（内存副本已被 `clearInMemory()` 清掉），要读到必须先插卡+密码认证开盘，而认证后第一件事就是补焚，故窗口内无暴露面。

**Commits**：`4eaae2d`（死线持久化 + 登录补焚 + 堵重启复看）/ `670492b`（发送方绝对 TTL + 两段弹框）。**未装机验证**。

## 最近操作：默认关闭 + 设置页开关（客户要求，2026-07-28）

M9.3 引入的「最近操作」原为常开：首页固定显示区块、各记录点无条件写日志。客户要求改为**默认不留痕**：

- **默认关闭**：首页不显示「最近操作」区块（**标题一并隐藏**，不是显示空列表），`OperationLogRepository.record()` 直接丢弃，后台不记录。
- **设置页开关**（安全操作分区，`SettingsStore.operationLogEnabled`，存手机本地 DataStore——与自动锁定同理：UI 偏好、需开盘前可读）。开启后**从开启那一刻起**记录。
- **关闭即清**：开关转 false 时清空内存并把卡上侧车覆盖成空；开关为 false 时的认证也**不读卡**、反而写空覆盖残留。故「关 → 再开」一定从零开始，不会浮出旧记录。
- **文案去「加密」**：日志条目不再提加解密——建立加密连接→建立连接、加密导出/解密导入→导出/导入、`KEY_UPDATE` 标签「文件加密密钥更新」→「文件密钥更新」。导出弹框内的 summary 文案不变（那里说的是用户当次选择的导出方式）。

## P2P 握手双向确认 + 文件收妥回执（协议 v2，客户现场日志 2026-08-02）

v4 §9 的握手是**单向**的：A 的临时公钥经二维码带外送达 B，B 只需把自己的公钥写进 socket 就能各自派生出会话密钥，**没有任何一方需要读到对方的回应**。文件传输同样是「写完 `FILE_END` 即算发送成功」。客户两台真机同一晚的日志（`midun_perf_20260802_184358/184418`）证明这两处都会**静默说谎**：

- **假连接**：`startListening` 原在握手成功后 `break` 出 accept 循环，可 `ServerSocket` 并没关——8888 仍 bind 着、只是再没人 accept。对端断线重连时内核照常完成三次握手，连接烂在 backlog 里；而 B 的握手只写不读，`connect()` 一返回就宣布 CONNECTED。现场即 19:01:50 B 报「会话建立」、19:02:01 报「发送文件成功」，而 A 侧同一时段**一条接收记录都没有**，直到 19:03:18 A 解冻才从 backlog 里捞出那条连接。
- **假送达**：`FILE_END` 写进本机内核缓冲区就返回成功，对端进程死了也一样。日志里「发送文件结束：成功，耗时 0s」与对端零接收并存。

**协议 v2（`ConnectionInfo.version = 2`）**：

- **握手双向确认**：B 发完自己的公钥后，再发一行 `AES-GCM(K, "MIDUN-CONNECTOR-OK")` 证明自己算出了同一把密钥（即确实持有二维码里的 A 公钥）；A 验过后回一行 `AES-GCM(K, "MIDUN-LISTENER-OK")`。**B 收到这一行才算连上**，它证明的是「A 真的 accept 并读到了我」，而不只是「TCP 通了」。
- **邀请码一次性 + 有效期在 A 侧判**：握手一旦成功，A **关掉 8888 监听并作废临时密钥**（原实现是 `break` 出循环却把端口留着 bind，正是上面那条假连接的温床）。此后任何连入（含掉线重连）拿到的是 ECONNREFUSED，扫码端如实提示「对方设备未在监听。邀请码可能已失效，请让对方重新生成」。有效期改由 A 自己记的 `listenerExpiresAt` 判（120 秒），**不信二维码里的 `exp`**——那是对端递过来的数，扫码方改了照样能连；邀请码页那个倒计时只在用户停留在该页时才走，用户一离开就没人执行了。**掉线不自动续听**：要再连必须由用户重新出码（用户 2026-08-02 拍板，邀请码必须是一次性的）。
- **文件收妥回执**：新增 `FILE_ACK` 帧（`{msgId, ok}`，会话密钥加密），收端校验完 sha256 后发出，发端等到它才标「已送达」；等不到（30s）即标失败并弃用当前通道。

**不做旧版本兼容**（用户 2026-08-02 定：两端一律装同一个包）：代码里没有任何按 `ver` 分岔的降级路径，`ConnectionInfo.version = 2` 只作标记。混版本连接会在握手确认那一步失败——这是刻意的，总好过又退回「连上了却谁也不知道对端在不在」。配套：`BuildConfig.GIT_COMMIT`（Gradle 取 `git rev-parse --short HEAD`，工作区脏则加 `-dirty`）写进诊断日志首行 `[diag] 进程启动｜版本 …｜构建 <commit>`，客户回传日志时一眼可核对跑的是不是我们刚发的那个包。
...
## 二维码：生成改用 ZXing，纠错升 M；删掉与 `addrs[0]` 重复的 `ipv6` 字段（`[qr]` 2026-08-11）

客户回报：邀请码识别 15 次有 2 次认不出，提示「图片 610×610，已补白重试」。8 月 9 日补的静区没能治本，因为病根在**编码器**上。

**根因（反编译 qrcode-kotlin 4.5.0 确认）**：`QRCodeBuilder` 两个我们从未设过的默认值——`errorCorrectionLevel` 默认 **LOW**（7%，四档最低），`maskPattern` 默认写死 **PATTERN000**，且全库没有任何掩码惩罚评分逻辑（`QRUtil` 只有 `getMask`，没有 lostPoint / bestMask）。而规范要求编码时把 8 种掩码逐个按惩罚分（大块同色、冒充定位图形的 1:1:3:1:1 行列、黑白失衡）评一遍取最优——掩码存在的意义就是打散坏图样。我们每张邀请码内容都是全新随机数据（临时 ECDH 公钥、随机 sid、变化的 exp），固定掩码下每隔若干张就会撞出一张模块排布很差的码；7% 的纠错余量又吃不下微信转发那一手 JPEG 压缩。**偶发、与内容相关、重试无效**，三个特征都对得上。

**改法**：

- **生成端换 ZXing `Encoder.encode()`**（`zxing-core` 本就在依赖里、此前一处未用），纠错 **L → M**，掩码由 ZXing 按规范评分挑选；静区改为按**模块**记（4 模块 × `QR_MODULE_PX`），不会再与模块尺寸失配。`qrcode-kotlin` 依赖已删除。
- **相册静图识别换 ZXing**，相机实时扫码**仍用 ML Kit**。ML Kit 是两段式（SSD 检测模型先定位、再解码），而分享出去的邀请码 PNG 有 87% 面积都是码本身，「一张除了码什么都没有」正是那个检测模型最不擅长的输入；ZXing 走经典路子（扫行找定位图形 → 透视采样 → 纠错解码），满幅纯码本就是它的标准输入。相机那条路相反：码只占取景框一部分，而模糊/暗光/斜角的宽容度是 ML Kit 更强，故不动。补白重试保留，但服务对象变成「存量老码 / 对方装的旧版 / 被裁掉静区的截图」——ZXing 对无静区的码宽容度更高但不免疫（`FinderPatternFinder` 按 1:1:3:1:1 扫行，定位图形贴到图像边缘时那一侧的白游程读不到）。
- **删掉 `ConnectionInfo.ipv6`**。它恒等于 `addresses[0]`，只为兼容「只认单地址」的旧版客户端。删的理由：一是名实不符（装的可能是 IPv4 字面量，见本文档「局域网 IPv4 回退」一节），二是白占载荷约 40 字节。实测同一份载荷 251 → 203 字节，M 级纠错下出图 650×650，与改前 L 级的 650×650 同密度——**纠错升级带来的增大被删字段完全抵消**。`candidates()` 随之删除，连接逻辑直接读 `addresses`；解析顺序与「先按本机能力过滤、再按序逐个 connect」的逻辑一律不变。
- **空地址表的处置从隐式改为显式**。原先 A 侧扫不到地址就填占位 `::1` 照常出码，那张码必然连不上，却要等对方扫完、`canReach` 判死才暴露，且报出来的是「对端设备未获取到 IPv6」——锅扣在没问题的那一端。现在两端各把一道：出码端 `generateConnectionInfo()` 候选为空直接抛 `NoLocalAddressException`（消息即给用户看的提示，由出码页接住显示）；解析端 `fromJson()` 把 `addrs` 列为必需且不许为空，缺了报「邀请码格式无效」。后者同时是**旧版二维码的拦截线**：8 月 11 日之前的码没有 `addrs`，到这里即判无效，提示重新出码——符合「两端强制装最新包」的约定。

**补记**：`addrs` 字段本身（多候选地址，`19a4330`）当初未登记进本文档，一并在此说明。JSON key 现为 `ver`/`sn`/`sid`/`tpk`/`exp`/`addrs`（原「M10.1 偏离1」一条所列的 `ipv6` 已不再存在）。**（2026-08-14 更新：`ver` 与 `sid` 亦已删除，key 现为 `sn`/`tpk`/`exp`/`addrs`，且整份 JSON 不再明文出现在码里，见文末「邀请码载荷混淆」一节。）**

## 扫码端在连接前自查邀请码有效期（`[network]` 2026-08-12）

现场拿到一张连不上的码，弹的是「连接超时：目标未响应。对方所在网络可能拦截了入站连接。」解出载荷才发现码本身早已过期（`exp` 09:52:09，扫的时候远晚于此）。**把「码过期了」报成了「对方网络有问题」**，而后一句用户照着什么也做不了。

根因是有效期只有出码端一道判据：`acceptOne()` 取本机记的 `listenerExpiresAt`（刻意不信码里的 `exp`——那是对端递过来的数，扫码方改了照样能连）。扫码端 `connectToContact()` 解出 `ConnectionInfo` 后**从没看过 `expiresAt`**，直接就 `connectTo`。这道判据的前提是「包能到达出码方」，而过期码的常态恰恰相反：码放了两分钟，出码方多半已换网、v6 临时地址已轮换、或息屏离网，SYN 根本没人回 → 扫码端白等一个 `ATTEMPT_TIMEOUT_MS`(6s) → 拿到 `SocketTimeoutException` → 翻译成那句网络文案。

**改法**：`ConnectionInfo.hasExpired()`（判据 + 10 秒时钟宽限），`connectToContact()` 解析成功后立刻自查，过期直接报「邀请码已过期，请让对方重新生成后再扫。」——不发 SYN，也不再占那 6 秒。

**宽限是必需的、方向也是定死的**：`exp` 是**出码方时钟**上的时刻，两台手机差多少不可控。宁可放过几张刚过期的码（照连即是，出码端那道权威判据兜底），也不能判死一张好码——判死了用户完全连不上，且没有任何自救手段。同理，本地这道只是**提前告知**，不承担安全职责：改码里的 `exp` 绕不过 `acceptOne()`。

链接粘贴那条入口同路（`parse()` 之后完全共用），不需要单独处理。

## 邀请码载荷混淆：通用扫码器扫出的是乱码（`[qr]` 2026-08-14）

客户要求：邀请码不能被微信等通用扫码器扫出明文。

**先把它是什么说清楚：这是混淆，不是加密，不构成任何安全边界。** 两台手机之间没有预共享秘密——B 要能扫开 A 的码，密钥就只能是内置常量，而内置常量随 APK 分发，拿到包的人必然能还原。所以：

- 它挡的是「路人拿微信随手一扫，就看见对方的安全卡 SN 与 IPv6 地址」——身份与位置，转发到群里就全暴露了；
- 它挡不住有心人。**UI 文案一个字都不许提「邀请码已加密」**（同 M12 那次「卡保密不动、App 叠 DEK，但不吹更安全」的口径）。

顺带说明为什么邀请码本身泄露不要紧：一次性消费、120 秒过期、握手要求对端确实持有码里那把临时公钥（`PROOF_FROM_CONNECTOR`）、SYN 还得在出码方停在监听页时到达。真正见不得人的是里面的 SN 和地址。

**格式**：二维码与邀请链接装**同一串**载荷 `base64url( IV(12) ‖ AES-GCM(版本字符 ‖ JSON) )`，密钥取 `SHA-256("MiDun-Invite-Payload-v1")`。链接只是在前面挂 `midun://invite?d=`，不再各编各的（原先链接是 base64(JSON)、二维码是裸 JSON，两条编码路径）。解不开的一切情形——不是我们的码、被截断、版本对不上、tag 不符——统一返回 null 报「邀请码格式无效」，因为对用户而言下一步都一样：让对方重新生成。

**为什么用 base64url 而不是把密文按字节模式直接塞进二维码**：后者省掉 33% 膨胀、码不变大，但要赌 ML Kit 与 ZXing 对非 UTF-8 字节串的编码处理完全一致——而这条链路 8 月 11/12 刚为「识别不出」折腾了两轮（换 ZXing 生成、加 PURE_BARCODE 才打到 300/300），不值得拿它去换体积。

**顺带删掉 `sid` 与 `ver` 两个字段**，把加密带来的膨胀抵掉一部分：

- `sid`（`sessionId`）是**死字段**：生成、写进 JSON、解析回来，此后全项目再没人读过。本文档「M10.2 遗留 / HKDF salt 取全零」那条曾写「如需更强会话绑定可后续把 sid 作 salt」——**该路线就此作废**：每次出码都新生成一对临时 ECDH 密钥，会话密钥本就不会复用，掺个随机数不增加任何东西；而拿旧码去连的场景已由「A 重新出码后 `listenerKeyPair` 即更换、B 算出的密钥对不上、握手确认当场失败」挡住。它却要在每张码里占约 20 字节，加密之后这正是最紧张的资源。
- `ver` 挪进加密后的首字符：版本是**载荷格式**的属性，不该混在业务 JSON 里。`ConnectionInfo.version` 字段一并删除（全项目从未读过）。

**代价**：载荷从 203 字节明文变为约 260 字符密文，二维码约从版本 10（57×57 模块）涨到版本 12（65×65），出图 650×650 → 730×730，纠错仍为 M。**这是本次唯一的真实风险**——码更密即更难拍，若现场反馈识别率下降，退路是把内层 JSON 换成紧凑二进制编码（去掉 key 名与十进制数字，估计可省三成），而不是退回明文。

按「不做旧版本兼容」处理：旧码在新版解不开（报格式无效），新码在旧版是乱码，两端一律装最新包。

**识别率实测（2026-08-14）**：新旧两种格式各 300 张随机载荷，渲染与解码两端按生产代码 1:1 复刻，原图 + JPEG q=0.9/0.7/0.5 各跑一遍，**共 2400 次解码零失败**。旧格式均长 185 字符（QR 版本 9–10，出图 610–650px），新格式均长 252 字符（版本 11–12，出图 730×730px）。台子与用法见 `tools/qr_bench/README.md`；原图那一半已固化为单元测试 `InvitePayloadQrTest`，改动即回归。

**这组数字量不到的两件事**（别拿零失败替它们背书）：一是**相机那条路用的是 ML Kit 而非 ZXing**，ML Kit 是 Android-only、离线台上跑不了（相册与链接那条路是 ZXing，已全覆盖）；二是**拍摄本身**——模糊、斜角、暗光、拍屏幕的摩尔纹，而码变密（57×57 → 65×65 模块，同样显示尺寸下每个模块更小）最先影响的正是这一项，只能真机验。

## 创建时间由 App 自己写进卡里（`[files]` 2026-08-15）

现场实测：文件与文件夹的创建日期一律显示 `--`。逐层排掉之后结论很干净——

- **不是我们的换算错**：`SFGetTime(fd, long[3])`、返回 0 为成功、`times[0]` 单位秒，逐字对过 SDK 自带示例 `FSDemoConsole.SFGetFileSizeAndCreateTime`（它的 `TimeStamp2Date` 同样 ×1000）。
- **不是 `SFOpenAsDir` 连累的**：把目录时间读取整个关掉打了一版对照包，文件日期照样 `--`。
- **也不是卡不支持**：`SFGetTime` 返回 `ret=0`，三个时间值都是 `-2209161600`。那个数正好是 **-25569 天**，而 25569 是 1899-12-30 到 1970-01-01 的天数，即 Delphi/OLE `TDateTime` 的零点——**卡上那三个字段从来没被写过**，SDK 把内部的「零日期」原样换算成了 Unix 秒。

也就是说：卡有时间字段，只是创建文件时没人往里写。`SFSetTime` 与 `SFGetTime` 对称，两个 ABI 的 `.so` 与 jar 里都在。

**改法**：`stampCreateTime(handle)` 在**创建那一刻**写入（创建/修改/访问三个都写当前秒）。落点选 `SFCreate`/`SFNewDir` 成功之后，而不是关句柄之前：一来那正是「创建时间」的定义，二来四条建文件的路（两条 `writeFile`、卡内复制 `copyWithinCard`、增量写 `streamCreate`）都收口在 `SFCreate` 上，在那里各打一次即全覆盖，增量写那条不必改调用方。目录经 `SFOpenAsDir` 拿句柄后同样打一次，拿不到就无声退化（日期显示 `--`）。

`createTimeOf` 里 `times[0] > 0` 那道判据正好把 Delphi 零日期这个哨兵值挡在外面，不必另加特判。

**认下的三件事**：

1. **只对以后的文件/文件夹有效**。卡上现存的那些，我们不知道它们何时创建，编一个出来是造假 —— 继续显示 `--`，按时间排序时落在一端。
2. **这在卡上多留了一份元数据**（何时导入过东西），此前是没有的。口径上与聊天记录、操作日志侧车里的时间戳一致，故接受。
3. **待真机验证**：后续写入若把创建时间也一并刷新（正常的文件系统只更新修改时间），那就得把打戳挪到关句柄之前。这一条只能装机看。

顺带：文件列表的「按时间排序」在此之前是**空转**的（所有 `createdAt` 都是 0，排出来实为名称兜底排）——与当初拒绝给文件夹时间选项的理由是同一条，这次一并治好。
