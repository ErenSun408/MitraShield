...
>
> - **基准代码**：`main` @ `a0c49b3`（M10.8 阅后即焚完成）。
> - **生成日期**：2026-06-10。
...
| 二维码：**压缩格式公钥 33 字节** | 🔶 | 实现用 `public.encoded` = **X.509/DER 未压缩（~91 字节）**，非需求的 33 字节压缩格式。功能等价但规格不符、二维码体积偏大 | `P2PSessionManager.generateConnectionInfo` |
| 二维码：sid 8字节 / JSON / base64 / 120s 失效 | ✅ | `generateRandomHex(8)`、org.json、`exp = now + 120_000`、120s 倒计时，均符合 | `ConnectionInfo` / `QrCodeScreen` |
| 扫码校验 **来源 / IPv6 / 签名** | 🔶 | UI 文案承诺"验证来源、IPv6、签名"，**实际无签名校验**，仅解析后直连 | `QrCodeScreen.ScanTab` |
...
3. **扫码"签名校验"是空头支票** — UI 写了验证签名，实际未做。要么实现签名校验，要么修正文案，否则验收易被抓。
4. **二维码公钥非"压缩 33 字节"** — 用了 X.509 未压缩编码。功能能通但规格不符、二维码偏大，改用压缩点编码可对齐。
